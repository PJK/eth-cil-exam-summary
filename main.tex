...
Translation invariance $\rightarrow$ shift filters; weights as filter masks, e.g. convolution: $F_{n,m}(\mathbf{x};\mathbf{w}) = \sigma(b + \sum_{k=-2}^2\sum_{l=-2}^{2}{w_{k,l}x_{n+k,m+1}})$. Reduce dimension: \{max, avg\}-pooling 
...
\begin{inlinelist}
	...
	\item u'te: $\mathbf{z}_{i^\star} \leftarrow \mathbf{z}_{i^\star} + \langle \mathbf{u}_{i^\star}, \mathbf{r} \rangle$
...
\begin{inlinelist}
  ...
  \item Column separation: For $n = 1, ..., N$: $z_n^{t+1} \in \argmin \|z\|_0$ 
  	st $\| \mathbf{x}_n - \mathbf{U}^t \mathbf{Z} \|_2 \leq \sigma \|  \mathbf{x}_n \|_2$
  \item Dictionary update step: $\mathbf{U}^{t+1} \in \argmin_\mathbf{U} \| \mathbf{X} - \mathbf{UZ}^{t+1} \|_F^2$, subject to $\forall l\in [L]:\|\mathbf{u}_l\|_2 = 1$ (non-separable, minimize residual by one column at a time)
\end{inlinelist}
