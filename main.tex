...
\titlespacing*{\sheader}{0pt}{0em}{0em}
\titlespacing*{\subsheader}{0pt}{0em}{0em}
...
\titleformat{name=\sheader}[block]
  {\sffamily}
  {}
  {0pt}
  {\subcolorsection}
\newcommand{\subcolorsection}[1]{%
	\colorbox{orange!10}{\parbox[t][0em]{\dimexpr\columnwidth-2\fboxsep}{\thesheader\ #1}}}


\titleformat{name=\subsheader}[block]
  {\sffamily}
  {}
  {0pt}
  {\subsubcolorsection}
\newcommand{\subsubcolorsection}[1]{%
	\colorbox{blue!10}{\parbox[t][0em]{\dimexpr\columnwidth-2\fboxsep}{\thesubsheader\ #1}}}
...
\newcommand{\sheader}[1]{\textbf{\underline{#1}}}
...
\begin{compactdesc}
	...
	\item[Jensen:] Convex $\varphi$: $\varphi ((\sum a_i x_i)/(\sum a_i)) \leq \frac{\sum a_i \varphi(x_i)}{\sum a_i}$, c've: $\geq$
\end{compactdesc}
...
\begin{inparaitem}[\color{red}\textbullet]
  ...
  \item if all evals nonzero: $\mathbf{A}^{-1} = \mathbf{Q} \boldsymbol{\Lambda}^{-1} \mathbf{Q}^{-1}$ and $(\boldsymbol{\Lambda}^{-1})_{i,i} = \frac{1}{\lambda_i}$
  \item if $\mathbf{A}$ symmetric: $A = \mathbf{Q} \boldsymbol{\Lambda} \mathbf{Q^\top}$ (and $\mathbf{Q}$ is orthogonal).
\end{inparaitem}

\sheader{Probability / Statistics}
\begin{inparaitem}[\color{red}\textbullet]
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item Linearity
	\item Convex/cavity
	\item Differen/integrability
	\item Inner product props
	\item Monotonicity
\end{inparaitem}
...
\begin{inlinelist}
	...
	\item eval Decomposition: $\boldsymbol{\Sigma} = \mathbf{U} \boldsymbol{\Lambda} \mathbf{U}^\top$, sort evals (and evecs) in descending order
	\item Select $K < D$, keep only the first $K$ evals and corresponding evecs $\Rightarrow \mathbf{U}_K, \boldsymbol{\lambda}_K$
...
\begin{inparaitem}
	...
	\item $\mathbf{U}$ columns are evecs of $\mathbf{A} \mathbf{A}^\top$, $\mathbf{V}$ columns are evecs of $\mathbf{A}^\top \mathbf{A}$, $\mathbf{D}$ diagonal elements are singular values (sqrts of evals of $\mathbf{A}^\top \mathbf{A}$ and $\mathbf{A} \mathbf{A}^\top$)
...
\sheader{Low-Rank approx -- $K$ largest evals and their evecs}
\begin{inparadesc}
...
\sheader{Clustering Stability}
\begin{inparaitem}[\color{red}\textbullet]
  \item Distance: $d(C,C') := \min_\Pi \frac{1}{2}\|Z-\Pi(Z')\|^2_F$ , $\Pi(Z
')$ = row perm. of $Z'$
  ...
  \item for $K$ clusters: $\text{stability} := 1 - \frac{r}{r_{rand}}$ (1 good), rand. clust. of equal size: $r_{rand} = \frac{K-1}{K}$.
\end{inparaitem}
...
\sheader{Expectation-Maximization (EM) for GMM}
\begin{inlinelist}
	\item Initialize $\boldsymbol{\pi}_k^{(0)}, \boldsymbol{\mu}_k^{(0)}, \boldsymbol{\Sigma}_k^{(0)}$ for $k = 1, \ldots, K$ and $t=1$.
	\item E: Pr$[z_{k,n} = 1 | \mathbf{x}_n] = q_{k, n} = \frac{\boldsymbol{\pi}_k^{(t-1)} \mathcal{N}(\mathbf{x}_n | \boldsymbol{\mu}_k^{(t-1)}, \boldsymbol{\Sigma}_k^{(t-1)})}{\sum_{j=1}^K \boldsymbol{\pi}_j^{(t-1)} \mathcal{N}(\mathbf{x}_n | \boldsymbol{\mu}_j^{(t-1)}, \boldsymbol{\Sigma}_j^{(t-1)})}$
	\item M: $\boldsymbol{\mu}_k^{(t)} := \frac{\sum_{n=1}^N q_{k,n} \mathbf{x}_n}{\sum_{n=1}^N q_{k,n}}$ \&  $\boldsymbol{\pi}_k^{(t)} := \frac{1}{N} \sum_{n=1}^N q_{k,n}$ \&  $\Sigma_k^{(t)} = \frac{\sum_{n=1}^N q_{k, n} (\mathbf{x}_n - \boldsymbol{\mu}_k^{(t)})(\mathbf{x}_n - \boldsymbol{\mu}_k^{(t)})^\top}{\sum_{n=1}^N q_{k,n}}$
	(From $\frac{\partial \ln \sum_j \sigma_j\mathcal{N}(..)}{\Sigma_k}$)
	\item $t = t + 1$ till $\| \log p(\mathbf{X} | \boldsymbol{\pi}^{(t)}, \boldsymbol{\mu}^{(t)}, \boldsymbol{\Sigma}^{(t)}) - \log p(\mathbf{X} | \boldsymbol{\pi}^{(t-1)}, \boldsymbol{\mu}^{(t-1)}, \boldsymbol{\Sigma}^{(t-1)}) \| < \epsilon$
\end{inlinelist}

\sheader{Model Order Selection (AIC / BIC for GMM)}
...
\sheader{GloVe (Weighted Square Loss)}
\begin{compactdesc}
...
\begin{compactdesc}
  ...
  \item 2. $\mathbf{y}_j^{new} \leftarrow \mathbf{y}_j + 2\eta f(n_{ij})(\log n_{ij} - \langle \mathbf{x}_i, \mathbf{y}_j \rangle)\mathbf{x}_i$
\end{compactdesc}
For $f = 1$: Solves $\min_{X, Y} \| M - X^{\top}Y\|^2_F$

\sheader{Latent Dirichlet Allocation}
\begin{inparaitem}
\item $p(u_i | \alpha) \propto = \Pi_{z = 1}^{K} u_{zi}^{a_k - 1}$.
\item Bayes: $p(x| V, \alpha) = \int p(x| V, u)p(u|\alpha)\textup{d}u$
\item Generation: For each $d_i$: sample $u_i$ (integrate out), sample slots ($z^t$, $w^t$ both categorical).
\end{inparaitem}
...
\begin{compactenum}[\color{red} 1.]
  \item Log-Likelihood: $L(\mathbf{U}, \mathbf{V}) = \sum_{i,j} x_{i,j}\log p(w_j|d_i) = \break \sum_{(i,j) \in X} \log \sum_{z=1}^K p(w_j|z)p(z|d_i)$
	...
	\item M-Steps: $p(z|d_i) = \frac{\sum_j x_{ij}q_{zij}}{\sum_j x_{ij}}\hspace{5pt}\&\hspace{5pt} p(w_j|z) = \frac{\sum_i x_{ij}q_{zij}}{\sum_{i,l}x_{il}q_{zil}}$
\end{compactenum}

\sheader{NMF Algorithm for quadratic cost function}
\begin{inlinelist}
...
\begin{inlinelist}
	...
	\item u'te $\mathbf{U}$ st: $(\mathbf{VV}^\top)\mathbf{U} = \mathbf{VX}^\top$
	\item project $u_{zi} = \max \{ 0, u_{zi} \}$
	\item u'te $\mathbf{V}$ st: $(\mathbf{UU}^\top)\mathbf{V} = \mathbf{UX}$
	\item project $v_{zj} = \max \{ 0, v_{zj} \}$
\end{inlinelist}

\section{Convolutional Neural Networks}
\textbf{Neurons}: $F_\sigma(\mathbf{x};\mathbf{w}) = \sigma(w_0 + \sum_{i=1}^M{x_iw_i})$. $L$ layers, 
$y = \sigma^L(W^L \sigma^{L-1} ... (W^1 x))$
\textbf{Output}: linear regression; $\mathbf{y} = \mathbf{W}^L\mathbf{x}^{L-1}$, 
binary classification; $y_1 = \text{P}[Y=1|\mathbf{x}] = \frac{1}{1 + \exp[-\langle \mathbf{w}_1^L,\mathbf{x}^{L-1}\rangle]}$, 
multiclass; $y_k = \text{P}[Y=k|\mathbf{x}]= \frac{\exp[\langle \mathbf{w}_k^L,\mathbf{x}^{L-1}\rangle]}{\sum_{m=1}^{K}{\exp[\langle \mathbf{w}_m^L, \mathbf{x}^{L-1}\rangle]}}$. \textbf{Loss function} $l(y, \hat{y})$: squared loss; $\frac{1}{2}(y - \hat{y})^2$, cross-entropy loss; $-y \log \hat{y} - (1-y)\log(1-\hat{y})$.
\textbf{Backpropagation}: $x^{l-1}_j$ affects $x^l_i$. 
Jacobian $J^l_{ij} = \frac{\partial x^l_i}{\partial x^{l-1}_j} = \dot{x}^l_i w_{ij}$ 
($\dot{x}^l_i = \frac{\textup{d}\sigma^l}{\textup{d}z}(z_i); z_i = \langle w^l_i, x^{l-1} \rangle$). 
Across $n$ layers: $\frac{\partial x^l_i}{\partial x^{l-n}_j} = \Pi_{i = l}^{l - n + 1} J^i$.
Weights contribution: $\frac{\partial L}{\partial w^l_{ij}} = \frac{\partial L}{\partial x^l_i} \dot{x}^l_i x^{l - 1}_j$

\sheader{Neural Networks for Images}
Translation invariance $\rightarrow$ shift filters; weights as filter masks, e.g. convolution: $F_{n,m}(\mathbf{x};\mathbf{w}) = \sigma(b + \sum_{k=-2}^2\sum_{l=-2}^{2}{w_{k,l}x_{n+k,m+l}})$. Reduce dimension: \{max, avg\}-pooling 

\section{Optimization}

\sheader{Coordinate Descent (update the $d$-th coord. per step)}
\begin{inlinelist}
...
\sheader{Gradient Descent (or Deepest Descent)}
...
\sheader{Stochastic Gradient Descent (SGD)}
...
\sheader{Projected Gradient Descent (Constrained Opt.)}
...
\sheader{Lagrangian Multipliers}
...
For \textbf{subderivative} $g \in \mathbb{R}^D$ of $f$ at $\mathbf{x}$: $f(\mathbf{y}) \geq f(\mathbf{x}) + g^\top(\mathbf{y}-\mathbf{x}) \ \forall \mathbf{y}$

\section{Sparse Coding}
\sheader{ONB} 
For $\mathbf{x}$ and \textsc{ONB} $\mathbf{U}$ compute $\mathbf{z} = \mathbf{U}^\top \mathbf{x} $. Approx $ \mathbf{\hat{x}} = \mathbf{U\hat{z}}$, $\hat{z}_i = z_i$ if $ \lvert z_i \rvert > \epsilon$ else 0.
RError $\|\mathbf{x}-\mathbf{\hat{x}}\|^2 = \sum_{d\notin\sigma}\langle\mathbf{x},\mathbf{u}_d\rangle ^2$.
Fourier -- global, wavelet -- local. PCA basis optimal for given $\Sigma$.

\sheader{Overcomplete Basis}
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item $m([\mathbf{B}, \mathbf{u}]) \geq \frac{1}{\sqrt{D}}$ if atom $\mathbf{u}$ is added to \textsc{ONB} $\mathbf{B}$
\end{inparaitem}

\sheader{Matching Pursuit (MP)}
...
\begin{inlinelist}
	\item $z \leftarrow 0, r \leftarrow x$
	\item while $\|\mathbf{z}\|_0 < K$ do
	\item select atom with smallest angle $i^\star = \argmax_i |\langle \mathbf{u}_i, \mathbf{r} \rangle|$
	\item u'te: $z_{i^\star} \leftarrow z_{i^\star} + \langle \mathbf{u}_{i^\star}, \mathbf{r} \rangle$
	\item u'te residual: $\mathbf{r} \leftarrow \mathbf{r} - \langle \mathbf{u}_{i^\star}, \mathbf{r} \rangle \mathbf{u}_{i^\star}$.
\end{inlinelist}
\textbf{Exact recovery} when: $K<1/2( 1+1/m(\mathbf{U}))$

\sheader{Compressive Sensing}
\begin{inparaitem}[\color{red}\textbullet]
  \item $\mathbf{x} \in \mathbb{R}^D$, $K$-sparse in \textsc{ONB} $\mathbf{U}$. $\mathbf{y} \in \mathbb{R}^M$ with $y_i = \langle \mathbf{w}_i, \mathbf{x}\rangle $: $M$ lin. combinations of signal; $\mathbf{y} = \mathbf{Wx} = \mathbf{WUz} = \mathbf{\theta z}$, $\theta \in \mathbb{R}^{M \times D}$
...
\sheader{Matrix Factorization by Iter Greedy Minimization}
\begin{inlinelist}
...
\sheader{Haar wavelets}
\begin{inparaitem}
	\item Systems are complete and pwise ortogonal
	\item Mother wlet: $\psi = (1, 1, -1, -1)^\top$, $\psi_{n, k} = 2^{n/2}\psi(2^nt - k)$
	\item Transform: $O(D)$ (cf. FFT $O(D \log D)$)
\end{inparaitem}
...
\sheader{Dual Ascent (Gradient Method for Dual Problem)}
...
\sheader{Dual Decomposition for Dual Ascent}: 
